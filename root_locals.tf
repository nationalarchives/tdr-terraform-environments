locals {
  ...
  ayr_terraform_deployer_roles = {
    ...
    staging = [
      ...
      format(
        "arn:aws:iam::%s:role/%s",
        module.ayr_configuration.account_numbers["prod"],
        "terraform-application-deployer"
      )
    ]
  ...
  waf_alb_target_groups = [module.keycloak_tdr_alb.alb_arn, module.consignment_api_alb.alb_arn, module.frontend_alb.alb_arn, module.transfer_service_tdr_alb[0].alb_arn]

  athena_metadata_checks_database_name = "athena-tdr-metadata-checks-${local.environment}"

  metadata_version_override = local.environment == "prod" ? "" : "TDRD-1475-"
