locals {
  ...
  internal_s3_encryption_key_arn = contains(["intg", "staging"], local.environment) ? module.s3_internal_kms_key.kms_key_arn : ""
  internal_bucket_key_enabled    = contains(["intg", "staging"], local.environment)

  // s3 upload bucket encryption
  upload_dirty_s3_encryption_key_arn = contains(["intg", "staging"], local.environment) ? module.s3_upload_kms_key.kms_key_arn : ""
  upload_dirty_bucket_key_enabled    = contains(["intg", "staging"], local.environment)
