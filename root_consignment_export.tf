...
module "consignment_export_task_policy" {
  ...
  policy_string = templatefile("./tdr-terraform-modules/iam_policy/templates/consignment_export_task_policy.json.tpl", { environment = local.environment, titleEnvironment = title(local.environment), aws_region = local.region, account = data.aws_caller_identity.current.account_id, kms_export_bucket_arn = module.s3_external_kms_key.kms_key_arn })
}
