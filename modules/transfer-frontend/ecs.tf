...
resource "aws_ecs_service" "frontend_service" {
  ...
  desired_count                     = var.environment == 1
  launch_type                       = "FARGATE"
...
resource "aws_iam_policy" "frontend_internal_s3_buckets" {
  name = "TDRFrontendEcsInternalS3Buckets${title(var.environment)}"
  policy = templatefile(
    "modules/transfer-frontend/templates/internal_s3_buckets_policy.json.tpl", {
      environment         = var.environment,
      titleEnvironment    = title(var.environment),
      account             = data.aws_caller_identity.current.account_id,
      kms_bucket_key_arns = var.internal_s3_kms_keys
  })
}

resource "aws_iam_role_policy_attachment" "frontend_internal_s3_buckets" {
  role       = aws_iam_role.frontend_ecs_task.name
  policy_arn = aws_iam_policy.frontend_internal_s3_buckets.arn
}
